...
- **Exposition — thumbnails** : les photos d'exposition génèrent maintenant un thumbnail 600px (comme tous les autres types d'activité) ; la grille affiche le thumbnail, la lightbox la pleine résolution.
...
**Tracking storageUsed pour les défis** : les défis n'étaient trackés nulle part dans `storageUsed`. Ajout du champ `defis?: number` dans `UserProfile.storageUsed` (user.model.ts). `DefiService.uploadPhoto()` incrémente désormais `storageUsed.defis` à chaque upload ; `DefiService.deletePhoto()` le décrémente à chaque suppression. `AuthService.recalculateStorage()` scanne maintenant les sous-collections `defis/{id}/photos` et écrit `storageUsed.defis`. `AuthService.deleteMemberData()` supprime aussi les photos de défis du membre (Storage + Firestore). Dashboard : `storageByCat`, `storageTotal` et `topMembers` intègrent désormais les défis ; une 5ᵉ barre rouge apparaît dans le graphe stockage.

**Corrections scan orphelins** : 5 fichiers Storage légitimes étaient incorrectement identifiés comme orphelins (images couverture stockées sur le doc principal et non dans la sous-collection photos) : `sorties/{id}/evenement-cover` (`imageEvenementPath`), `themes/{id}/couverture` (`photoCouverturePath`), `oneshots/{id}/couverture` (`photoCouverturePath`), `defis/{id}/couverture.webp` (`photoCouverturePath`). Le préfixe `articles/` est maintenant inclus dans le scan orphelins avec les `couvertureStoragePath` des articles dans le known set. Le compteur de progression passe de 4 à 5 étapes.

### Dashboard admin — section Maintenance & nettoyage photos (2026-07-09)

Suppression de la section "Événements à venir" du dashboard (redondant avec le calendrier) et du compteur "à venir" dans la KPI card. Ajout d'un nouveau bloc **Maintenance** en bas du dashboard avec 5 panneaux : Thèmes du mois, One-shots, Défis, Sorties, et Orphelins Storage. Chaque panneau dispose d'un date picker (valeur par défaut : −2 ans), d'un bouton "Analyser" (preview à sec), d'un bouton "Nettoyer" (confirmation via `ConfirmService`) et d'une **checkbox "Conserver le top 3"** (cochée par défaut). Logique top-3 : conserve les 3 photos les mieux votées (ex-aequo inclus) ; si le 3ᵉ score est 0 (aucun vote), toutes les photos sont conservées. One-shots : top 3 par thème interne. Défis : top 3 global. Sorties : suppression totale des photos (pas de votes). Votes supprimés après chaque nettoyage. Barre de progression pendant l'exécution. Panel **Orphelins Storage** : `listAll()` récursif sur les 4 préfixes (themes/, oneshots/, defis/, sorties/), cross-référencé avec les `storagePath` Firestore ; affiche la liste des fichiers orphelins (max 100) avant suppression. Implémenté dans le nouveau `CleanupService` (`providedIn: root`). Règles mises à jour : `firestore.rules` — suppression des votes par admin (`|| isAdmin()`) sur les 3 collections de votes ; `storage.rules` — `allow list` pour utilisateurs connectés sur les 4 préfixes (`{allPaths=**}`) pour autoriser `listAll()`.
