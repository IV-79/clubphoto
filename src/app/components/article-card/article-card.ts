...
@Component({
  ...
  template: `
    <div class="card" (click)="clicked.emit(article())">
      @if (isEditor()) {
        <a
          class="edit-btn"
          [routerLink]="['/membre/articles', article().id, 'editer']"
          mat-icon-button
          title="Modifier"
      ...
      <div class="body">
        @if (isEditor() && article().statut !== 'publie') {
          <span
            class="statut-badge"
            [class.brouillon]="article().statut === 'brouillon'"
            [class.expire]="article().statut === 'expire'"
          >
  ...
  styles: [
    `
      ...
      .placeholder-info {
        background: linear-gradient(135deg, rgba(25, 118, 210, 0.08), rgba(25, 118, 210, 0.22));
        color: #1976d2;
      }
      .placeholder-encart {
        background: linear-gradient(135deg, rgba(123, 31, 162, 0.08), rgba(123, 31, 162, 0.22));
        color: #7b1fa2;
      }
      .placeholder-expo {
        background: linear-gradient(135deg, rgba(230, 81, 0, 0.08), rgba(230, 81, 0, 0.22));
        color: #e65100;
      }
      .placeholder-evenement {
        background: linear-gradient(135deg, rgba(46, 125, 50, 0.08), rgba(46, 125, 50, 0.22));
        color: #2e7d32;
      }
      .placeholder-annonce {
        background: linear-gradient(135deg, rgba(245, 124, 0, 0.08), rgba(245, 124, 0, 0.22));
        color: #f57c00;
      }
      .type-badge {
        position: absolute;
