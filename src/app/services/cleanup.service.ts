...
export class CleanupService {
  ...
  async previewDefis(beforeDate: string, keepTop3 = true): Promise<CleanupPreview> {
    ...
    for (const defi of old) {
      ...
      if (!keepTop3) {
        ...
        const countMap = new Map<string, number>();
        votes.forEach((v) =>
          v.photoIds.forEach((pid) => countMap.set(pid, (countMap.get(pid) ?? 0) + 1)),
        );
        const keep = this.keepSet(
          photos.map((p) => p.id),
          countMap,
  ...
  async previewOrphans(): Promise<OrphanPreview> {
    ...
    for (const theme of themes) {
      ...
      subs.forEach((s) => {
        known.add(s.storagePath);
        if (s.thumbnailPath) known.add(s.thumbnailPath);
      });
    }
    ...
    articles.forEach((a) => {
      if (a.couvertureStoragePath) known.add(a.couvertureStoragePath);
    });
    ...
    return {
      ...
      orphanPaths: orphans.slice(0, 100),
    };
  }
  ...
  async executeOrphans(preview: OrphanPreview): Promise<{ deleted: number }> {
    ...
    let deleted = 0;
    this.progress.set({
      label: 'Suppression orphelins…',
      current: 0,
      total: full.orphanPaths.length,
    });
