...
export class CharteService {
  ...
  mustAcceptCharte$ = combineLatest([this.authService.currentUserProfile$, this.charteDoc$]).pipe(
    map(([user, charte]) => {
  ...
  mustAcceptCgu$ = combineLatest([this.authService.currentUserProfile$, this.cguDoc$]).pipe(
    map(([user, cgu]) => {
